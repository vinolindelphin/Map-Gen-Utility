...
SIMPLIFY_TOLERANCE_M = 500  # 0 disables simplification
...
KPI_CONFIG = {
    ...
    # 2) AEPS_GTV_IN_LACS (already in lakhs)
    "AEPS_GTV_IN_LACS": {
...
def run_query_cached(sql: str, month_date: str, state_name: str) -> pd.DataFrame:
    ...
    return client.query(sql, job_config=job_cfg).result().to_dataframe(create_bqstorage_client=False)
...
def run_query(kpi_key: str, month_date: str, state_name: str) -> pd.DataFrame:
    ...
    if "pincode" not in df.columns:
        raise ValueError("Result must include 'pincode'. Got: " + ", ".join(df.columns))
...
# Session state to control “run only on click”
if "run_id" not in st.session_state:
    st.session_state.run_id = 0
if "last_params" not in st.session_state:
    st.session_state.last_params = None
if "pending_changes" not in st.session_state:
    st.session_state.pending_changes = True  # nothing has run yet
...
with st.sidebar:
    ...
    month_str = st.text_input("Month (YYYY-MM)", value="2025-10", on_change=mark_changed)
    month_param = f"{month_str}-01"
    state = st.selectbox("State", STATES, index=0, on_change=mark_changed)

    # Single button to run
...
if clicked:
    ...
    st.session_state.last_params = (kpi_key, month_param, state)
    should_run = True
else:
    # If filters changed after last run, do not run; show info
    if st.session_state.pending_changes:
        st.info("Adjust filters, then click **Generate map**.")
    # If nothing changed since last run, we don’t re-query (prevents that ‘running…’ overlay)
    elif st.session_state.last_params == (kpi_key, month_param, state):
        # show previous map again (nothing to do)
        pass
    else:
        # Params differ but button not clicked -> don’t run
        st.info("Filters changed. Click **Generate map** to refresh the map.")

# When we should run, do all heavy work inside a spinner
if should_run:
    cfg = KPI_CONFIG[kpi_key]
    ...
    with st.spinner("Generating map…"):
        ...
        # Legend
        legend = [("#d9d9d9", "0 / missing")]
        def _fmt_edge(v):
            if kpi_key == "AEPS_GTV_IN_LACS": return f"{v:.0f} L"
            if kpi_key == "Trxn_SMAs": return f"{int(v)}"
            return f"{v/100000:.0f} L"
        for i in range(1, len(bins)-1):
            legend.append((colors[i-1], f"{_fmt_edge(bins[i-1])} – {_fmt_edge(bins[i])}"))
        legend.append((colors[-1], f"> {_fmt_edge(bins[-2])}"))
        legend_html = """
        <div style="position: fixed; bottom: 20px; left: 20px; z-index: 9999; font-size: 12px;
                    background: white; padding: 10px 12px; border:1px solid #ccc; border-radius:4px;">
          <b>{title}</b><br>
          {rows}
        </div>
        """.format(
            title=f"{kpi_key} • {unit_name}",
            rows="".join(
                f'<i style="background:{c};width:12px;height:12px;float:left;margin-right:6px;opacity:0.9"></i>{t}<br>'
                for c, t in legend
            )
        )
        m.get_root().html.add_child(folium.Element(legend_html))

    # Title + map (unique key uses run_id so it always refreshes only on click)
    st.markdown(f"### {kpi_key} • {month_str} • {state}")
    st_folium(m, height=720, width=None, key=f"map_{st.session_state.run_id}")
